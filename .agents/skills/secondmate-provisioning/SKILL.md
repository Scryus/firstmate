...
`bin/fm-harness.sh secondmate-model` and `bin/fm-harness.sh secondmate-effort` print the optional 2nd/3rd tokens from any line whose harness token is present and not `default` - including a harness-only line, which stays empty for model/effort exactly as before (backward-compat). Only when `config/secondmate-harness` is absent, blank/comment-only, or its harness token is literally `default` do they instead print a provider-aware default for the harness `resolve_secondmate` resolves to - falling through to `config/crew-harness` and then the primary's own harness the same way a plain spawn would - or nothing when that resolved harness has no known-safe default: `claude-sonnet-5 medium` for `claude`, and `gpt-5.6-luna medium` for `pi`/`pi-signed` only when the live Pi primary's own current model provider is `openai-codex`.
