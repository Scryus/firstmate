...
state/               runtime records and signals; gitignored
  ...
  .main-model         live Pi primary's own current model as "<provider>/<id>"; written only by .pi/extensions/fm-branch-supervision.ts on every session_start and model_select event; absent or unreadable means unknown, never a guessed provider; bin/fm-harness.sh reads it to pick provider-aware secondmate defaults (docs/configuration.md "config/secondmate-harness")
